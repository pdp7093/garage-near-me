...
@router.get("/chrome-stats")
def get_chrome_stats(
    current_admin: models.Admin = Depends(get_current_admin),
    db: Session = Depends(get_db)
):
    pending_garages = db.query(models.GarageRequest).filter(
    ...
    pending_payouts = db.query(models.PlatformPayoutRequest).filter(
        models.PlatformPayoutRequest.status == "pending"
    ).count()

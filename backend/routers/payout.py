...
def process_payout_request_admin(
    ...
    if action_data.action == "approve":
        ...
        if garage:
            ...
            old_dues = float(garage.pending_platform_dues)
            
            # Deduct dues (floor to 0)
            garage.pending_platform_dues = max(0.0, float(garage.pending_platform_dues) - float(payout_req.amount))
            
            # Mark trial completed if they had >= 500 dues
            if not garage.has_completed_trial and old_dues >= 500.0:
                garage.has_completed_trial = True

            # Unlock and reset grace period
            if garage.has_completed_trial:
                if garage.pending_platform_dues <= 0.0:
...
# ──────────────────────────────────────────
# 5. ADMIN: GENERATE BILLING STATEMENTS (15-Day Cycle)
# POST /api/payouts/admin/generate-billing-statements
# ──────────────────────────────────────────

def generate_billing_statements_internal(db: Session):
    # Find active, verified, post-trial garages with dues > 0 and no lock/grace
    garages = db.query(models.Garage).filter(
        models.Garage.is_active == True,
        models.Garage.is_verified == True,
        models.Garage.has_completed_trial == True,
        models.Garage.pending_platform_dues > 0.0,
...
@router.post("/admin/generate-billing-statements")
def generate_billing_statements(
    x_admin_key: str = Header(None),
    db: Session = Depends(get_db)
):
    """
    Admin triggers 15-day billing statement generation (1st and 15th of the month).
    Finds all active, verified garages who have COMPLETED THEIR TRIAL,
    have outstanding dues > 0, and no active grace period/lock,
    and starts a strict 48-hour grace period for payment.
    """
    verify_admin(x_admin_key)

    count, deadline = generate_billing_statements_internal(db)

    return {
        "message": f"Successfully generated 15-day billing statements for {count} mechanics.",
        "count": count,

  ...
  form.addEventListener('submit', async (e) => {
    ...
    container.innerHTML = `
      <div style="padding: 30px 10px; text-align: center;">
        <div class="spinner-border text-danger" role="status" style="width: 45px; height: 45px; margin-bottom: 20px; border-width: 3px;">
          <span class="visually-hidden">Loading...</span>
        </div>
        <h5 style="font-weight: 600; color: #F1F5F9;">Submitting Payment Proof...</h5>
      </div>
    `;
    ...
    try {
      ...
      showCreditLockOverlay(duesAmount);
      setTimeout(() => {
        const errorText = document.getElementById('utrErrorMsg');
        errorText.textContent = err.message || 'Payment submission failed. Please try again.';
